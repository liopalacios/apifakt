...
*/
            ...
            String stringMoneda = mapMoneda.get(invoice.getCodigoMoneda().toUpperCase())!=null?
                    ...
                    //invoice.getCodigoMoneda()!=null?invoice.getCodigoMoneda().equalsIgnoreCase("USD")?"Dólares Americanos":"Soles":"Soles";

            attributes.put(ConstantesSunat.ATTRIBUTE_LANGUAGE_LOCALE_ID, "1000");
            appendChild(doc, invoiceRootElement, "cbc:Note", GenerateLetraNumber.Convertir(invoice.getImporteTotalVenta().setScale(2,BigDecimal.ROUND_HALF_UP).toString(),stringMoneda,true) , attributes);
            ...
            appendChild(doc, invoiceRootElement, "cbc:LineCountNumeric", invoice.getItems().size());
            ...
            if (StringUtils.isNotBlank(invoice.getCodigoTipoOperacion())
                ...
                for (Anticipo anticipo : invoice.getAnticipos()) {
                    ...
                    appendChild(doc, additionalDocument, "cbc:DocumentTypeCode", anticipo.getTipoDocumentoAnticipo());
            ...
            if (StringUtils.isNotBlank(invoice.getDireccionOficinaEmisor())) {
                ...
                CompanyEntity companyEntity = companyRepository.findByRuc(invoice.getRucEmisor());
                Element cacAddressLine = appendChild(doc, cacRegistrationAddress, "cac:AddressLine");
                appendChild(doc, cacAddressLine, "cbc:Line", companyEntity.getDireccion());
            }
            Element cacCountry = appendChild(doc, cacRegistrationAddress, "cac:Country");
            attributes.clear();
            attributes.put(ConstantesSunat.ATTRIBUTE_LIST_AGENCY_NAME, "United Nations Economic Commission for Europe");
            attributes.put(ConstantesSunat.ATTRIBUTE_LIST_ID, "ISO 3166-1");
            attributes.put(ConstantesSunat.ATTRIBUTE_LIST_NAME, "Country");
            appendChild(doc, cacCountry, "cbc:IdentificationCode", "PE",attributes);
            ...
            for (PaymentVoucherLine item : invoice.getItems()) {
                ...
                appendChild(doc, cacInvoiceLine, "cbc:InvoicedQuantity", UtilFormat.format(item.getCantidad()), attributes)
